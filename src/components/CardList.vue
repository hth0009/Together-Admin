  ...
  >
    <div class="search-wrapper">
      ...
      <div class="filter-section-toggle" v-if="filters.length > 0">
        <i
          ...
          :class="{selected: toggleFilterSection}"
        >filter_list</i>
      </div>
    </div>
    <div class="filter-wrapper">
      <div class="selected-filters" :class="{toggled: toggleFilterSection}">
        <div class="filters">
          <div
            v-for="(filterKey, index) in Object.keys(selectedFilters)"
            :key="index"
            class="filter"
            @click="toggleFilter(filterKey, selectedFilters[filterKey], '', '', '')"
          >
            <ejs-tooltip
              :position="'BottomCenter'"
              :content="filterHash[filterKey + selectedFilters[filterKey]].title"
            >
              <i
                v-show="filterHash[filterKey + selectedFilters[filterKey]].icon != ''"
                class="material-icons noselect selected"
              >{{filterHash[filterKey + selectedFilters[filterKey]].icon}}</i>
              <div
                class="short selected"
                v-show="filterHash[filterKey + selectedFilters[filterKey]].icon == ''"
              >{{filterHash[filterKey + selectedFilters[filterKey]].short}}</div>
            </ejs-tooltip>
          </div>
        </div>
        <div
          class="clear"
          v-show="Object.keys(selectedFilters).length != 0"
        ...
        <div v-for="filterType in listFilters" :key="filterType.type">
          ...
          <div class="filters">
            <div
              ...
              :key="filter.value"
              class="filter"
              @click="toggleFilter(filterType.key, filter.value, filter.icon, filter.title, filter.short)"
            >
              <ejs-tooltip :position="'BottomCenter'" :content="filter.title">
                <i
                  v-show="filter.icon != ''"
                  class="material-icons noselect"
                  :class="{selected: selectedFilters[filterType.key] === filter.value}"
                >{{filter.icon}}</i>
                <div
                  v-show="filter.icon == ''"
                  class="short noselect"
                  :class="{selected: selectedFilters[filterType.key] === filter.value}"
                >{{filter.short}}</div>
              </ejs-tooltip>
            </div>
    ...
    <div data-simplebar class="card-boxes" v-if="!loading" v-on:scroll="manageScroll">
      <div
        class="card-wrapper nothing-here"
        v-if="filteredCards.length == 0 && !!!cardSearch"
      >{{emptyMessage}}</div>
      <div class="card-wrapper" v-for="(card, index) in filteredCards" :key="card[fields.id]">
        <div
          class="header"
          v-if="card['header'] && (index == 0 || filteredCards[index - 1][fields.header] != card[fields.header])"
        >{{card.header}}</div>
        <div
        ...
        >
          ...
          >
            ...
            <div
              v-if="card[fields.subtitle]"
              class="subtext"
            >{{card[fields.subtitle].substring(0, 30) + ""}}</div>
            <div
              v-if="card[fields.subtitle2]"
              class="subtext-2"
            >{{card[fields.subtitle2].substring(0, 30) + ""}}</div>
          </div>
          <!-- <div v-if="card['body']" class="body">
            {{card['body'].substring(0, 250) + ""}}
          </div>-->
          <div
            v-if="hasButtonOnCard && !!!buttonColor"
          ...
          <div
            ...
            class="card-button"
          >
            <i class="material-icons" style="color:green" @click="$emit('onApprove', id = card[fields.id])">check</i>
            <i class="material-icons" style="color:red" @click="$emit('onDeny', id = card[fields.id])">clear</i>
          </div>
        </div>
      </div>
    </div>
    <div class="card-boxes loading" v-if="loading">
      <div v-for="(card, index) in Array(17)" :key="index" class="card-box loading">
        <div class="profile-pic"></div>
      </div>
    </div>
    <div class="add-new" v-if="hasAddNew" @click="addNew">
  ...
  methods: {
    ...
    manageScroll({ target: { scrollTop, clientHeight, scrollHeight } }) {
      ...
      const previousScroll =
        this.previousScroll === -1 ? 0 : this.previousScroll;
      const speed = Math.max(Math.abs(previousScroll - scrollTop) / 5, 0.5);
      // console.log(speed)
      if (
        scrollTop + clientHeight >= scrollHeight - 1 &&
        this.maxHeight > maxHeightFloor
      ) {
...
.all-filters {
  ...
  margin-bottom: 0px;
  padding: 0px;
  transition: max-height 0.15s ease-out, overflow-y 1s 1s ease,
    margin 0.15s ease-out, padding 0s 0.15s ease-in-out;
  overflow: hidden;
}
.all-filters.toggled {
  max-height: 500px;
...
.card-box.loading:nth-child(2) {
  animation-delay: 0.25s;
}
.card-box.loading:nth-child(3) {
  animation-delay: 0.5s;
}
.card-box.loading:nth-child(4) {
  animation-delay: 0.75s;
}
.card-box.loading:nth-child(5) {
  animation-delay: 1s;
}
.card-box.loading:nth-child(6) {
  animation-delay: 1.25s;
}
.card-box.loading:nth-child(7) {
  animation-delay: 1.5s;
}
.card-box.loading:nth-child(8) {
  animation-delay: 1.75s;
}
.card-box.loading:nth-child(9) {
  animation-delay: 2s;
}
.card-box.loading:nth-child(10) {
  animation-delay: 2.25s;
}
.card-box.loading:nth-child(11) {
  animation-delay: 2.5s;
}
.card-box.loading:nth-child(12) {
  animation-delay: 2.75s;
}
.card-box.loading:nth-child(13) {
  animation-delay: 3s;
}
.card-box.loading:nth-child(14) {
  animation-delay: 3.25s;
}
.card-box.loading:nth-child(15) {
  animation-delay: 3.5s;
}
.card-box.loading:nth-child(16) {
  animation-delay: 3.75s;
}
.card-box.loading:nth-child(17) {
  animation-delay: 4s;
}
.card-box.loading:nth-child(18) {
  animation-delay: 4.25s;
}
.card-box.loading:nth-child(19) {
  animation-delay: 4.5s;
}
.card-box.loading:nth-child(20) {
  animation-delay: 4.75s;
}
.card-box.loading:nth-child(21) {
  animation-delay: 5s;
}
.card-box.loading:nth-child(22) {
  animation-delay: 5.25s;
}
.card-box.loading:nth-child(23) {
